...
function Index({ children }) {
  ...
  return (
    <div className={styles.root + " pt-3 md:p-5 max-h-screen "}>
      ...
      <div className={styles.centerbar + " md:py-2 px-2 md:px-6 max-h-full text-center"}>
        {children}
        <button className={homeStyles.Button + ` mt-9 md:hidden rounded-md py-1 px-3 text-sm ${router.pathname==='/' && 'hidden'}` } onClick={handleBackToHome} >
          {"<-"} Back to Home
        </button>
      </div>
