...
function SecondRow() {
  ...
  const completedPrecentage = (tasks.filter((task) => task.completed)).length / tasks.length;
  const uncompletedPrecentage = tasks.filter((task) => !task.completed);
  return (
    <div className={styles.secondRowContainer + " "}>
      <div className={"mr-3 p-5 rounded-xl"}>
        <div className={styles.contentBox + ' text-center'}>
            <h1 className={'text-xl md:text-2xl'}>{percentage} Tasks</h1>
