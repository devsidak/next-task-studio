...
function SecondRow() {
  ...
  const CalculateCompletedPercentage = useMemo(() => {
    ...
    <div className={styles.secondRowContainer + " "}>
      <div className={"mr-3 p-5 rounded-xl"}>
        ...
        <div className={styles.progressbarContainer}>
          <CircularProgressbar
            value={(CalculateCompletedPercentage / tasks.length) * 100}
            text={`${(CalculateCompletedPercentage / tasks.length) * 100}%`}
            strokeWidth={12}
            styles={buildStyles({
              strokeLinecap: "round",
              textSize: "16px",
      ...
      <div className={" ml-3 p-5 rounded-xl"}>
        <div className={styles.contentBox + " text-center"}>
          <h1 className={"text-xl md:text-2xl"}>
            {tasks.length - CalculateCompletedPercentage} Tasks
          </h1>
        ...
        <div className={styles.progressbarContainer}>
          <CircularProgressbar
            value={((tasks.length - CalculateCompletedPercentage) / tasks.length) * 100}
            text={`${percentage}%`}
            strokeWidth={12}
            styles={buildStyles({
              strokeLinecap: "round",
