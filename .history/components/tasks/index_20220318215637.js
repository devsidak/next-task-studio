...
function Index({task, loading, setLoading, removeTodo, HandleupdateState}) {
  ...
  return (
    <li className={styles.task + ' p-9 m-4 '}>
        <div className={''}>
            <input disabled={loading} onChange={(e) => handleCompleteUncompleteTask(e, task)} checked={check} className={styles.checkbox} type="checkbox" />
        </div>
        <div>
            <p>{task.task}</p>
        </div>
