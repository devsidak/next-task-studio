...
function Tasks() {
  ...
  const renderNotasksView = () => {
    return(
      <div className={'py-6 m-4 min-h-full  align-middle flex-center text-center border-2 border-gray-700 rounded-3xl'}>
        ...
        <h3 className={'text-center text-xl text-slate-400'}>No Tasks For Today</h3>
        <h3 className={'text-center text-md p-2 text-slate-400'}>Enjoy Your Day!</h3>
  ...
  return <div>
    {tasks.length !== 0 ? <ul>{renderTasks()}</ul> : <>{renderNotasksView()}</>  }</div>;
}
