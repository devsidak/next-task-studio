...
export const addTodo = (task) => (dispatch) => {
  ...
  .then(function (value) {
    console.log("from ls - ", value);
    nextStateTasks = [newTask, ...value];
    console.log("nextStateTasks - ",nextStateTasks);
  ...
  localforage.setItem('todos', JSON.stringify(nextStateTasks).then(() => {
    console.log('todos saved to local storage - ', nextStateTasks);
  }).catch(function (err) {
    console.log(err);
  });
