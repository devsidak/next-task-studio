...
export const addTodo = (task) => (dispatch) => {
  ...
  const todos = localforage.getItem('todos');
...
export const SyncFromLocalStorage = () => (dispatch) => {
  ...
  localforage.getItem('todos').then((todos) => {
    ...
    console.log("todos from local storage", todos===null?[]:todos);
    // dispatch({
    //   type: typ.SYNC_FROM_LOCAL_STORAGE,
    //   payload: {
    //     todos
    //   },
    // });
  ...
  //   });
  // }).then(() => {
  //   console.log("SyncFromLocalStorage completed");
  // }).catch((err) => {
  //   console.log(err);
  // });
// };
